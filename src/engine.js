// 하프타임의 기적 — 결정론 규칙 엔진
// 원칙: 외부 API 0, 순수 클라이언트. 같은 카드 조합 = 같은 결과 (시드 고정).
...
export const CARDS = [
  ...
  { id: 'supersub', name: '슈퍼서브 투입', icon: '🦸', atk: 3, def: 0,  sta: 2,  desc: '벤치의 금강산 투입. 체력이 쌩쌩하다.', risk: '교체 카드 소진' },
  { id: 'counter',  name: '역습 올인',     icon: '⚡', atk: 3, def: -1, sta: 0,  desc: '내려섰다가 한 방에 지평선의 스피드로 찌른다.', risk: '점유를 완전히 내준다' },
  { id: 'setpiece', name: '세트피스 집중', icon: '📐', atk: 2, def: 1,  sta: 0,  desc: '코너킥·프리킥에서 백두혁의 머리를 노린다.', risk: '필드골 생산력은 그대로' },
  { id: 'captain',  name: '캡틴 독려',     icon: '🗣️', atk: 1, def: 1,  sta: 2,  desc: '이현로가 팀을 다시 뛰게 만든다.', risk: '전술 변화는 없음' },
  { id: 'gamble',   name: '수비수 공격 가담', icon: '🎲', atk: 4, def: -3, sta: -1, desc: '차돌민까지 올린다. 마지막 도박.', risk: '역습 한 방이면 끝장' },
...
// good = 변주 배열 — 같은 카드가 여러 장면에 나와도 문장이 복붙처럼 반복되지 않게 (2026-07-22 눈검수 fix)
const CARD_SCENES = {
  press:    { good: ['전방 압박이 통했다! 상대 골킥을 최전방에서 끊어낸다', '나포효가 골키퍼까지 쫓아간다. 상대 빌드업이 완전히 질식', '높은 위치에서 볼 탈취! 곧바로 슈팅 각이 열린다'], bad: '압박에 다리가 풀렸다. 후반 막판 라인이 무너진다' },
  overload: { good: ['왼쪽 오버로드! 한결까지 올라가 숫자로 부순다', '왼쪽에서 2대1, 3대2 — 상대 풀백이 비명을 지른다'], bad: '오른쪽 뒷공간이 텅 비었다. 아즈리카가 그 틈을 노린다' },
  longball: { good: ['롱볼 한 방이 수비 뒷공간에 떨어진다', '강철벽의 롱킥이 단숨에 최전방으로 — 경합!'], bad: '롱볼이 번번이 걷혀 나온다. 점유율이 30%대' },
  threetop: { good: ['스리톱 총공세! 박스 안이 붉은 유니폼으로 가득하다', '세 명의 공격수가 동시에 침투한다. 수비가 셋을 다 못 본다'], bad: '중원이 실종됐다. 세컨볼을 전부 내준다' },
  ...
  supersub: { good: ['금강산 투입! 벤치가 준비한 비장의 카드', '금강산이 쌩쌩한 다리로 수비 사이를 헤집는다'], bad: '' },
  counter:  { good: ['뺏자마자 지평선이 폭발적인 스피드로 하프라인을 돌파한다', '역습 한 방! 지평선의 질주에 수비가 따라오지 못한다'], bad: '내려선 채 공만 바라본다. 슈팅이 안 나온다' },
  setpiece: { good: ['코너킥, 연습한 그림이다! 백두혁이 솟구친다', '프리킥 상황 — 준비한 사인 플레이가 나온다'], bad: '' },
  captain:  { good: ['이현로가 소리친다. 팀 전체 활동량이 눈에 띄게 올라간다', '주장의 박수 한 번에 관중이 다시 끓는다'], bad: '' },
  gamble:   { good: ['차돌민까지 박스 안으로! 수비수의 머리!', '수비수가 최전방에 서 있다. 벼랑 끝 올인'], bad: '차돌민이 올라간 사이, 등 뒤가 고속도로다' },
  wingswap: { good: ['윙 스위칭에 상대 풀백이 완전히 길을 잃었다', '좌우가 바뀌자 매치업이 무너진다. 반박자 빠른 크로스'], bad: '' },
}
const goodText = (cardId, minute) => {
  const g = CARD_SCENES[cardId]?.good
  if (!g) return ''
  return Array.isArray(g) ? g[minute % g.length] : g
}
...
export function simulate(rawIds, interventionId = 'hold') {
  ...
  const rnd = mulberry32(seedFromCards(ids))
  const scenes = []
  let us = 0, them = 2
  ...
  for (const minute of slots) {
    ...
    const pGoalUs = Math.min(0.34, 0.03 + (atk + iAtk) * 0.017 + (late ? 0.04 : 0))
    const pGoalThem = Math.min(0.4, Math.max(0.03, 0.15 - (def + iDef) * 0.011) + fatigue)
    const r = rnd()
    ...
    const good = goodText(cardId, minute)
    if (r < pGoalUs) {
      us++
      ...
      scenes.push({ minute, type: 'goal_them', card: cardId,
        text: `${minute}' 💔 실점… ${cs.bad || '역습 한 방을 허용한다'} — ${us}-${them}` })
  ...
  if (us > them) { outcome = 'win';  headline = `기적이다! ${us}-${them} 대역전승!` }
  else if (us === them) { outcome = 'draw'; headline = `${us}-${them}. 벼랑 끝에서 살아 돌아왔다. 승부는 연장으로.` }
  else { outcome = 'loss'; headline = `${us}-${them}. 뒤집지 못했다. 하지만 후반의 그 15분은 진짜였다.` }
  ...
  return { scenes, us, them, outcome, headline, rating: rating.toFixed(1), stats: { atk, def, sta }, notes, intervention: iv.id }
}
