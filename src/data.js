...
export const SCENARIOS = {
  korea: {
    ...
    basePos: { // 실제 선발 3-4-3: 스리백(이한범·김민재·이기혁) + 윙백(설영우/이태석)
      gk1: { x: 6, y: 50 },
      df1: { x: 20, y: 25 }, df2: { x: 18, y: 50 }, df3: { x: 20, y: 75 },
      df4: { x: 40, y: 13 }, mf3: { x: 40, y: 87 },
      mf1: { x: 43, y: 38 }, mf2: { x: 43, y: 62 },
      fw1: { x: 66, y: 25 }, fw2: { x: 70, y: 50 }, fw3: { x: 66, y: 75 },
    },
    ...
    names: { SPD: '황희찬', SUB: '손흥민', TGT: '김민재', CAP: '김민재', GAM: '이한범', LW: '이태석', GK: '김승규', MAGIC: '이강인' },
    cardOverrides: {
      press:    { name: '하이프레스', desc: '황희찬·오현규가 남아공의 빌드업을 최전방에서 문다. 낮은 블록을 세우기 전에 뺏는다.', risk: '체력 소모 큼 — 막판에 다리가 풀리면 역습에 무너진다',
                  sceneGood: ['하이프레스가 통했다! 남아공이 블록을 세우기 전에 볼을 뺏어낸다', '황희찬이 골키퍼까지 쫓아간다 — 남아공 빌드업이 질식', '높은 위치 볼 탈취! 밀집수비가 정렬되기 전에 슈팅 각이 열린다'] },
      overload: { name: '이태석 좌측 과부하', desc: '왼쪽 윙백 이태석 쪽에 숫자를 몰아 밀집수비를 한쪽부터 비틀어 연다.', risk: '반대쪽 뒷공간 노출 — 모레미가 노리는 바로 그 공간',
                  sceneGood: ['왼쪽 과부하! 이태석까지 올라가 숫자로 부순다', '왼쪽에서 2대1, 3대2 — 남아공 블록이 한쪽으로 쏠린다'] },
      longball: { name: '오현규 타겟 롱볼', desc: '빌드업 생략. 김승규의 롱킥을 타겟 오현규에게 바로 붙인다. 밀집수비 앞 지공 대신 다이렉트로.', risk: '점유율을 내주고 세컨볼 싸움이 된다',
                  sceneGood: ['김승규의 롱킥이 단숨에 오현규에게 — 경합에서 이겨낸다!', '롱볼 한 방이 남아공 블록 뒤로 떨어진다'] },
      threetop: { name: '공격 올인 전환', desc: '이강인을 최전방까지 올려 사실상 4톱. 밀집수비를 물량으로 누른다.', risk: '중원 장악력 약화 — 역습 한 방에 취약해진다',
                  sceneGood: ['공격 전원 투입! 남아공 박스 안이 태극 유니폼으로 가득하다', '네 명이 동시에 침투한다. 5백도 다 못 본다'] },
      linedown: { name: '역습 차단 블록', desc: '라인을 내려 마세코가 달릴 공간부터 지운다. 남아공의 유일한 무기를 꺾는 수비.', risk: '공격 숫자 감소 — 0-0으로도 32강이지만, 정말 그걸로 만족하는가', fateGuard: 0.8,
                  sceneGood: ['내려선 블록이 마세코의 역습 공간을 지워버린다', '남아공 역습이 시작되기도 전에 막힌다. 달릴 공간이 없다'] },
      tempo:    { name: '황인범 템포 조율', desc: '황인범이 내려와 점유를 잡고 남아공 역습의 김을 뺀다. 서두르지 않는다.', risk: '시간은 흐른다 — 무승부면 32강이지만 불안하다면?',
                  sceneGood: ['황인범이 템포를 쥔다. 경기가 우리 리듬으로 돌아온다', '점유율이 다시 올라간다. 남아공 역습의 김이 빠진다'] },
      counter:  { name: '맞역습', desc: '남아공이 역습하러 나올 때가 기회다. 뺏는 순간 황희찬의 스피드로 그들의 뒷공간을 찌른다.', risk: '점유를 완전히 내주는 수 싸움',
                  sceneGood: ['맞역습! 뺏자마자 황희찬이 폭발적인 스피드로 하프라인을 돌파한다', '남아공이 올라온 순간 — 황희찬의 질주에 수비가 따라오지 못한다'] },
      setpiece: { name: '세트피스: 김민재 타깃', desc: '밀집수비를 부수는 가장 확실한 루트. 코너킥·프리킥에서 김민재의 머리를 노린다.', risk: '필드골 생산력은 그대로',
                  sceneGood: ['코너킥, 준비한 그림이다! 김민재가 솟구친다', '이강인의 킥이 정확히 김민재 머리 위로 떨어진다'] },
      captain:  { name: '주장의 라인 정리', desc: '김민재가 소리치며 라인을 다잡는다. 집중력이 돌아온다.', risk: '전술 변화 자체는 없다',
                  sceneGood: ['김민재가 소리친다. 팀 전체 활동량이 눈에 띄게 올라간다', '주장의 박수 한 번에 관중이 다시 끓는다'] },
      gamble:   { name: '이한범 전진 도박', desc: '제공권 좋은 이한범까지 최전방으로 올린다. 스리백을 스스로 허무는 마지막 승부수.', risk: '마세코의 역습 한 방이면 끝장',
                  sceneGood: ['이한범까지 박스 안으로! 수비수의 머리를 노린다!', '수비수가 최전방에 서 있다. 벼랑 끝 올인'] },
      wingswap: { name: '이강인-황희찬 스위칭', desc: '좌우를 바꿔 남아공 수비의 매치업 계산을 흔든다. 밀집수비도 사람이 지킨다.', risk: '적응에 10분이 필요하다',
                  sceneGood: ['좌우 스위칭에 남아공 풀백이 완전히 길을 잃었다', '이강인이 반대 측면에서 공을 잡자 마크가 꼬인다'] },
    },
